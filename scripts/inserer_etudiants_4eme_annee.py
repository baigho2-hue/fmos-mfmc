...
# -*- coding: utf-8 -*-
"""
Script pour insérer des étudiants en 4ème année du DESMFMC dans la base de données.
Utilise l'email comme username et le mot de passe "etudiant123".
Crée les ResultatAnneeDES nécessaires pour les années 1, 2 et 3 (statut 'admis').

Usage:
    python scripts/inserer_etudiants_4eme_annee.py
"""
import os
import sys
...
ETUDIANTS = [
    {
        ...
        'prenom': 'Fadiè',
        'telephone': '76064223',
        'email': 'traoredrsoumaonofadie@gmail.com',
    },
    {
        'nom': 'TOURE',
        'prenom': 'Issoufou',
        'telephone': '92207492',
        'email': 'issoufoutoure92@gmail.com',
    },
    {
        'nom': 'COULIBALY',
        'prenom': 'Adama B',
        'telephone': '76190531',
        'email': 'drcoulibaly601@gmail.com',
    },
    {
        'nom': 'TRAORE',
        'prenom': 'Amadou',
        'telephone': '73061153',
        'email': 'amadoust14@gmail.com',
    },
    {
        'nom': 'KONE',
        'prenom': 'Nabi Issa',
        'telephone': '79097649',
        'email': 'nabiissakone@gmail.com',
    },
    {
        'nom': 'Soumaoro',
        'prenom': 'Mohamed N',
        'telephone': '76698558',
        'email': 'mohamednarembasoumaoro@gmail.com',
    },
    {
        'nom': 'MAIGA',
        'prenom': 'Seydou',
        'telephone': '75314760',
        'email': 'maigaseydou579@gmail.com',
    },
]
...
def trouver_classe_4eme_annee(formation):
    """Trouve la classe 4ème A pour le DESMFMC."""
    # Chercher d'abord par nom exact
    classe = Classe.objects.filter(
        formation=formation,
        nom__icontains='4ème A',
        actif=True
    ).first()
    
    # Si pas trouvé, chercher par année 4
    if not classe:
        classe = Classe.objects.filter(
            formation=formation,
            annee=4,
    ...
    if not classe:
        raise SystemExit(f"❌ Classe '{CLASSE_NOM}' ou classe année 4 introuvable pour DESMFMC.")
...
def creer_ou_mettre_a_jour_etudiant(etudiant_data, formation, classe):
    ...
    if utilisateur:
        ...
        utilisateur.classe = CLASSE_NOM  # Utiliser "4ème A" comme demandé
    ...
    utilisateur = Utilisateur.objects.create_user(
        ...
        classe=CLASSE_NOM,  # Utiliser "4ème A" comme demandé
...
def main():
    """Fonction principale."""
    print("=" * 70)
    print("Insertion des étudiants en 4ème année du DESMFMC")
    print("=" * 70)
    
    # Trouver la formation et la classe
    formation = trouver_formation_desmfmc()
    classe = trouver_classe_4eme_annee(formation)
    
    print(f"\n✅ Formation: {formation.nom}")
    print(f"✅ Classe: {classe.nom} (année {classe.annee})")
    print(f"✅ Mot de passe: {PASSWORD}")
    print(f"\nNombre d'étudiants à traiter: {len(ETUDIANTS)}\n")
    print("ℹ️  Note: Les mémoires (thème, protocole, document final) seront ajoutés ultérieurement.\n")
    ...
    for idx, etudiant_data in enumerate(ETUDIANTS, 1):
        ...
        try:
            ...
            # Créer les résultats pour les années 1, 2 et 3
            for annee in [1, 2, 3]:
    ...
    print(f"✅ Résultats années 1, 2 et 3 créés/mis à jour: {stats['resultats_crees']}")
    if stats['erreurs'] > 0:
    ...
    print("\nℹ️  Les mémoires (thème, protocole, document final) pourront être ajoutés ultérieurement.")
