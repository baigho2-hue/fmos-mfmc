...
# -*- coding: utf-8 -*-
"""
Script pour insérer des étudiants en 2ème année du DESMFMC dans la base de données.
Utilise l'email comme username et le mot de passe "etudiant123".
Crée les ResultatAnneeDES nécessaires pour le processus de validation.

Usage:
    python scripts/inserer_etudiants_2eme_annee.py
"""
import os
import sys
...
ETUDIANTS = [
    {
        ...
        'prenom': 'Simbo',
        'telephone': '75274885',
        'email': 'Simbocoulibaly500@gmail.com',
    },
    {
        'nom': 'Agnidé',
        'prenom': 'Maroufatou Moussa',
        'telephone': '76045297',
        'email': 'yayiagnid@yahoo.fr',
    },
    {
        'nom': 'Kébé',
        'prenom': 'Mahamadou',
        'telephone': '92576698',
        'email': 'Drkebemahamadou@gamil.com',
    },
    {
        'nom': 'Sangaré',
        'prenom': 'Aminata',
        'telephone': '76891918',
        'email': 'Sangareami99@gmail.com',
    },
    {
        'nom': 'Diawara',
        'prenom': 'Nafatoumata',
        'telephone': '79185115',
        'email': 'Tim04diawara@gmail.com',
    },
    {
        'nom': 'Coumaré',
        'prenom': 'Moussa',
...
def trouver_classe_2eme_annee(formation):
    """Trouve la classe 2ème A pour le DESMFMC."""
    # Chercher d'abord par nom exact
    classe = Classe.objects.filter(
        formation=formation,
        nom__icontains='2ème A',
        actif=True
    ).first()
    
    # Si pas trouvé, chercher par année 2
    if not classe:
        classe = Classe.objects.filter(
            formation=formation,
            annee=2,
    ...
    if not classe:
        raise SystemExit(f"❌ Classe '{CLASSE_NOM}' ou classe année 2 introuvable pour DESMFMC.")
...
def creer_ou_mettre_a_jour_etudiant(etudiant_data, formation, classe):
    ...
    utilisateur = Utilisateur.objects.create_user(
        ...
        classe=CLASSE_NOM,  # Utiliser "2ème A" comme demandé
...
def creer_resultat_annee_1(etudiant, formation):
    """Crée le ResultatAnneeDES pour l'année 1 avec statut 'admis'."""
    defaults = {
        'decision': 'admis',
...
def main():
    """Fonction principale."""
    print("=" * 70)
    print("Insertion des étudiants en 2ème année du DESMFMC")
    print("=" * 70)
    
    # Trouver la formation et la classe
    formation = trouver_formation_desmfmc()
    classe = trouver_classe_2eme_annee(formation)
    ...
    for idx, etudiant_data in enumerate(ETUDIANTS, 1):
        ...
        try:
            ...
            # Créer le résultat pour l'année 1
            resultat, cree = creer_resultat_annee_1(utilisateur, formation)
            if cree:
                stats['resultats_crees'] += 1
            print(f"  ✅ Résultat année 1: {'créé' if cree else 'déjà existant'}")
    ...
    print(f"✅ Résultats année 1 créés/mis à jour: {stats['resultats_crees']}")
    if stats['erreurs'] > 0:
